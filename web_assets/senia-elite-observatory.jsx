...
const T = { bg: "#06080f", p: "#0c0e18", b: "#1a1d2e", surface: "#12141f", elevated: "#181b2a", tx: "#e2e4f0", dim: "#6b6f88", m: "#2e3148", a: "#38bdf8", w: "#fb923c", r: "#f472b6", g: "#34d399", rd: "#ef4444", mono: "'IBM Plex Mono','JetBrains Mono',monospace", font: "'Inter','Noto Sans SC',system-ui,sans-serif" };
...
const Spark = ({ d, th, c = T.a, h = 100, label }) => {
  ...
  return <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" style={{ borderRadius: 8 }}>
    <defs>
    ...
    {ty !== null && ty > 0 ? <>
      ...
      <text x={w - pad - 2} y={ty - 3} textAnchor="end" fill={T.rd} fontSize="8" fontFamily={T.mono} opacity=".7">TH {th?.toFixed(2)}</text>
    </> : null}
    <polyline points={pts} fill="none" stroke={c} strokeWidth="2.5" strokeLinejoin="round" strokeLinecap="round" style={{ filter: `drop-shadow(0 2px 4px ${c}30)` }} />
    {arr.length <= 30 && arr.map((v, i) => {
      const x = pad + (i / (arr.length - 1)) * (w - 2 * pad);
      const y = h - pad - ((v - mn) / rg) * (h - 2 * pad);
      return <circle key={i} cx={x} cy={y} r="2.5" fill={v > (th || Infinity) ? T.rd : c} stroke={T.bg} strokeWidth="1" />;
    })}
...
const Radar = ({ l, c, h, s = 180 }) => {
  ...
  const ax = [{ n: "dL", v: Math.abs(l), a: -90, label: `dL ${l >= 0 ? "+" : ""}${l.toFixed(2)}` }, { n: "dC", v: Math.abs(c), a: 30, label: `dC ${c >= 0 ? "+" : ""}${c.toFixed(2)}` }, { n: "dH", v: Math.abs(h), a: 150, label: `dH ${h >= 0 ? "+" : ""}${h.toFixed(2)}` }];
  ...
  return <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`}>
    <defs><radialGradient id={uid}><stop offset="0%" stopColor={T.a} stopOpacity=".15" /><stop offset="100%" stopColor={T.a} stopOpacity=".03" /></radialGradient></defs>
    ...
    <polygon points={pts.map((v) => `${v.x},${v.y}`).join(" ")} fill={`url(#${uid})`} stroke={T.a} strokeWidth="2" strokeLinejoin="round" style={{ filter: `drop-shadow(0 0 8px ${T.a}30)` }} />
    {pts.map((v, i) => <circle key={i} cx={v.x} cy={v.y} r="4.5" fill={T.a} stroke={T.bg} strokeWidth="2" />)}
...
export default function EliteObservatory() {
  ...
  return <div style={{ minHeight: "100vh", background: T.bg, color: T.tx, fontFamily: T.font }}>
    <style>{`
      ...
      .senia-tab:hover { background: rgba(255,255,255,0.04) !important; }
      .senia-tab-active::after { content: ""; position: absolute; bottom: 0; left: 20%; right: 20%; height: 2px; border-radius: 2px; }
    `}</style>
    <header style={{ position: "sticky", top: 0, zIndex: 3, background: `linear-gradient(180deg,${T.surface}f0,${T.bg}e0)`, backdropFilter: "blur(12px)", padding: "16px 18px 12px", borderBottom: `1px solid rgba(255,255,255,0.06)` }}>
      ...
      <div style={{ marginTop: 12, display: "flex", gap: 3, overflowX: "auto", paddingBottom: 2 }}>{tabs.map((x) => <button key={x.id} className={`senia-tab ${tab === x.id ? "senia-tab-active" : ""}`} onClick={() => setTab(x.id)} style={{ padding: "7px 12px", borderRadius: 8, border: "none", background: tab === x.id ? `${x.c}12` : "transparent", color: tab === x.id ? x.c : T.dim, fontSize: 10, fontWeight: 700, whiteSpace: "nowrap", cursor: "pointer", boxShadow: tab === x.id ? `0 0 12px ${x.c}15` : "none" }}>{x.n}</button>)}</div>
    </header>
    <main style={{ padding: "12px 16px 24px" }}>
      ...
      <Panel t="Sampling Cockpit" c={gc}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>{["s", "f"].map((k) => <div key={k} style={{ border: `1px solid ${T.b}`, borderRadius: 8, padding: 8, background: T.bg }}><div style={{ fontSize: 10, color: T.dim, marginBottom: 6 }}>{k === "s" ? "Sample RGB" : "Film RGB"}</div>{["r", "g", "b"].map((ch) => <div key={`${k}_${ch}`} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}><span style={{ width: 12, fontSize: 10, color: T.dim, textTransform: "uppercase" }}>{ch}</span><input type="number" min={0} max={255} value={(k === "s" ? sample : film)[ch]} onChange={(e) => setRgb(k, ch, e.target.value)} style={{ ...inp, padding: "4px 6px" }} /></div>)}</div>)}</div><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 20, flexWrap: "wrap" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 56, height: 56, borderRadius: 12, background: `rgb(${sample.r},${sample.g},${sample.b})`, border: `2px solid ${T.a}40`, boxShadow: `0 6px 24px rgba(${sample.r},${sample.g},${sample.b},.3)`, margin: "0 auto" }} />
          <div style={{ fontSize: 8, color: T.dim, marginTop: 5, fontFamily: T.mono }}>L{sLab.L.toFixed(1)} a{sLab.a.toFixed(1)} b{sLab.b.toFixed(1)}</div>
          <div style={{ fontSize: 8, color: T.a, marginTop: 1 }}>Sample</div>
        </div>
        <div style={{ width: 40, height: 16, borderRadius: 10, background: `linear-gradient(90deg,rgb(${sample.r},${sample.g},${sample.b}),rgb(${film.r},${film.g},${film.b}))`, border: `1px solid rgba(255,255,255,0.06)` }} />
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 56, height: 56, borderRadius: 12, background: `rgb(${film.r},${film.g},${film.b})`, border: `2px solid ${T.r}40`, boxShadow: `0 6px 24px rgba(${film.r},${film.g},${film.b},.3)`, margin: "0 auto" }} />
          <div style={{ fontSize: 8, color: T.dim, marginTop: 5, fontFamily: T.mono }}>L{fLab.L.toFixed(1)} a{fLab.a.toFixed(1)} b{fLab.b.toFixed(1)}</div>
          <div style={{ fontSize: 8, color: T.r, marginTop: 1 }}>Film</div>
        </div>
